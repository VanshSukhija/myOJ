    ...
    <div className='w-full min-h-screen flex flex-col gap-2'>
      ...
      {standings && standings.length > 0 &&
        <table className='w-full px-2 mx-auto'>
          <thead>
            <th className='w-fit px-2 border border-slate-500'>#</th>
            <th className='w-fit px-2 border border-slate-500'>Username (email)</th>
            <th className='w-fit px-2 border border-slate-500'>Penalty</th>
            {
              standings[0].problems.map((problem, index) => (
                <th key={index} className='w-[10%] px-2 border border-slate-500 overflow-clip'>{problem.problemName}</th>
              ))
            }
          </thead>
          <tbody>
            {
              standings.map((participant, index) => (
                <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-800' : 'bg-black'}`}>
                  <td className='text-center border border-slate-500 py-1'>{index + 1}</td>
                  <td className='text-center border border-slate-500 py-1'>{participant.username} ({participant.email})</td>
